...
public class FormatCheckUtils {
    ...
    public static boolean checkFormatTell(String tell1, String tell2, String tell3) {
        ...
        if (tell1.equals("02")) {
            ...
            if (tell3.length() != 4) { //서울지역(02)국번 입력때의 전화 번호 형식유효성 체크
                return false;
            }
        } else {
            if (tell2.length() != 3 && tell2.length() != 4) {
    ...
    public static boolean checkFormatTell(String tellNumber) {
        ...
        if (tell.charAt(1) == '2') {   //서울지역 (02)국번의 경우일때
            ...
            if (tell.length() != 10 && tell.length() != 11) {
                return false;
            }
            if (tell.length() == 10) {
    ...
    public static boolean checkFormatMail(String mail1, String mail2) {
        ...
        for (int i = 0; i < mail1.length(); i++) {
            if (mail1.charAt(i) <= 'z' && mail1.charAt(i) >= 'a') {
                ...
                continue;
            } else if (mail1.charAt(i) == '-' || mail1.charAt(i) == '_' || mail1.charAt(i) == '+' || mail1.charAt(i) == '.') {
                continue;
            } else {
                return false;
            }
        ...
        for (int i = 0; i < mail2.length(); i++) {
            if (mail2.charAt(i) <= 'z' && mail2.charAt(i) >= 'a') {
                continue;
            } else if (mail2.charAt(i) <= '9' && mail2.charAt(i) >= '0') {
                continue;
    ...
    public static boolean checkLength(String value, int minLength, int maxLength) {
        ...
        return checkMinLength(value, minLength) && checkMaxLength(value, maxLength);
    }
    ...
    public static boolean checkDayOfWeek(String value) {
        if (value == null) {
            return false;
        }

        // 월 ~ 금요일까지만 문자열 확인
        if (!"MON".equals(value) && !"TUE".equals(value) && !"WED".equals(value) && !"THU".equals(value) && !"FRI".equals(value)) {
            return false;
        }
    ...
    public static boolean checkPeriod(String startDate, String endDate) {
        ...
        try {
            dateFormatParser.setLenient(false);
            Date from = dateFormatParser.parse(startDate);
            Date to = dateFormatParser.parse(endDate);
    ...
    public static boolean checkDateType(String value) {
        return "year".equals(value) || "month".equals(value) || "day".equals(value) || "hour".equals(value);
    }

    public static boolean checkMemberId(String value) {
        if (value == null) {
            return false;
        }

        return Pattern.matches("^[a-z]{1}+[a-z|0-9]{5,14}$", value);
    }

    public static boolean containsIdInPassword(String id, String pwd) {
        if (id == null || pwd == null) {
            return false;
        }

        for (int i = 0; i + 3 < id.length(); i++) {
            String idPart = id.substring(i, i + 4);
            if (pwd.contains(idPart)) {
                return true;
            }
        }
        return false;
    ...
    public static boolean checkMemberPassword(String value) {
        if (value == null) {
            return false;
        }
        ...
        return Pattern.matches("^(?=.*[0-9])(?=.*[a-zA-Z])[0-9a-zA-Z]{10,16}$", value);
    }

    public static boolean checkMemberName(String value) {
        if (value == null) {
            return false;
        }

        return Pattern.matches("^[가-힣a-zA-Z]{2,10}$", value);
    }
}
