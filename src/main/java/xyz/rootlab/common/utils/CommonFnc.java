...
import xyz.rootlab.common.config.etc.EgovMultiLoginPreventor;
...
//            }
    ...
    public static String getRequestBody(HttpServletRequest request) {
        ...
        try {
            ...
            while((line = reader.readLine()) != null) {
                json.append(org.springframework.util.StringUtils.trimAllWhitespace(line));
            }
        }catch(Exception e) {
    ...
    public static void logging(HttpServletRequest request, HttpServletResponse response) {
        ...
        log.debug("### request - [{}][{}][{}][{}]", clientIp, requestURI, method, contentType);
        log.debug("### requestBody - [{}]", reqBody);
        log.debug("### responseBody - {}", resBody);
    }

    public static void downloadExcel(HttpServletRequest request, HttpServletResponse response, String fileName, String sheetName, String[] columnTitle, String[][] content) throws Exception {
        XSSFWorkbook workBook = new XSSFWorkbook();

        XSSFFont defaultFontStyle = workBook.createFont();
        defaultFontStyle.setFontHeight(10);
        defaultFontStyle.setColor(IndexedColors.BLACK.index);
        ...
        titleFontStyle.setFontHeight(12);
        titleFontStyle.setColor(IndexedColors.BLACK.index);
        titleFontStyle.setFontName("맑은 고딕");

        titleFontStyle.setBold(true); // 폰트굵게
        ...
        detailFontStyle.setFontHeight(10);
        detailFontStyle.setColor(IndexedColors.BLACK.index);
        detailFontStyle.setFontName("맑은 고딕");
        detailFontStyle.setBold(true); // 폰트굵게
        ...
        if (userAgent.contains("MSIE 5.5")) { // MS IE 5.5 이하
            response.setHeader("Content-Disposition", "filename=" + (URLEncoder.encode(fileName, "UTF-8") + ".xlsx").replaceAll("\\+", "\\ ").replaceAll("%0d", " ").replaceAll("%0a", " ") + ";");
        } else if (userAgent.contains("MSIE")) { // MS IE (보통은 6.x 이상 가정)
            response.setHeader("Content-Disposition",
                    "attachment; filename=" + (java.net.URLEncoder.encode(fileName, "UTF-8") + ".xlsx").replaceAll("\\+", "\\ ").replaceAll("%0d", " ").replaceAll("%0a", " ") + ";");
        } else if (userAgent.contains("Trident") || userAgent.contains("Edge")) { // MS IE 11, Edge
            response.setHeader("Content-Disposition",
                    "attachment; filename=" + (java.net.URLEncoder.encode(fileName, "UTF-8") + ".xlsx").replaceAll("\\+", "\\ ").replaceAll("%0d", " ").replaceAll("%0a", " ") + ";");
        } else { // 모질라나 오페라
            response.setHeader("Content-Disposition",
                    "attachment; filename=" + (new String(fileName.getBytes("UTF-8"), "latin1") + ".xlsx").replaceAll("\\+", "\\ ").replaceAll("\r", " ").replaceAll("\n", " ") + ";");
        }
